...
import { ModalGeneral, Text, View } from "@/app/components";
import useRegister from "@/hooks/useRegister";
import Link from "next/link";
import { useEffect, useState } from "react";
import { useForm } from "react-hook-form";
import { HiOutlineMail } from "react-icons/hi";
import SectionSignUpView from "../SectionSignUpView/SectionSignUpView";
import { EmailSentView } from "./EmailSentView";

export const SignUpAndDiveIn = () => {
  ...
  return (
    <>
    <SectionSignUpView
      classView={`max-w-[479px] h-fit lg:h-fit ${!isEmailSentViewOpen ? '' : 'hidden'}`}
      classViewBG="h-fit backdrop-blur-[32px] bg-[linear-gradient(180deg,rgba(14,12,21,0.9)_0%,rgba(14,12,21,0)_100%)] flex flex-col"
    >
      <form 
      onSubmit={handleSubmit(onSubmit)}
      >
         <div className="font-chakra-petch font-bold text-[40px] leading-[48px] text-white">Sign Up and Dive In!</div>
          <div className="mt-4 font-sora font-semibold text-[18px] leading-[26px] text-white">Unlock New Adventures</div>
          ...
          <div className="mt-4 font-sora text-[16px] leading-[24px] text-white text-center">Already have an account? <Link
            href={`/sign-in`}
            className="inline-block text-gunnies-purple font-bold cursor-pointer ">Log in</Link></div>
     ...
     <ModalGeneral
            modalOpen={isEmailSentViewOpen}
            modalBody={<EmailSentView
            onClose={() => setIsEmailSentViewOpen(false)}
            />}
            modalTitle={undefined}
            modalDescription={undefined}
            setModalOpen={setIsEmailSentViewOpen}
